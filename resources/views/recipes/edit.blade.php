<x-app-layout>
  ...
  <form action="{{ route('recipe.update', ['id' => $recipe['id']]) }}" method="POST" class="w-10/12 p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
    ...
    <div class="flex justify-center">
      <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">レシピ更新</button>
    </div>
    <!-- underline  -->
    <hr class="my-4">
  ...
  <!-- 削除ボタン -->
  <form action="{{ route('recipe.destroy', ['id' => $recipe['id']]) }}" method="POST" class="w-10/12 p-4 my-6 mx-auto">
    @csrf
    @method('DELETE')
      <button id="delete" type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">レシピ削除</button>
  </form>
</x-app-layout>