<x-app-layout>
  ...
  <!-- ログインしないとレビューできない機能 -->
  @guest
    <p class="text-center text-gray-500 mt-3">レビューを投稿するには<a href="{{ route('login') }}" class="text-green-500 font-bold text-xl"> 【ログイン】 </a>してください。</p>
  @endguest
  @auth
  @if($is_reviewed)
    <p class="text-center text-gray-500 mb-4">レビューは投稿済みです。
      RecipeControllerのis_reviewedをtrueで連続投稿w
    </p>
    @elseif($is_my_recipe)
    <p class="text-center text-gray-500 mb-4">自分のレシピにはレビューできません。<br>
    show.blade.phpの&#64;elseif($is_my_recipe)を変更で投稿可能ｗ
    </p>
    @else
    <!-- reviewsレビュー機能 -->
